...
if ~isempty(minmaxIdx(diff(minmaxIdx)==1))
    minmaxIdx = minmaxIdx(diff(minmaxIdx)==1);
end
if ~isempty(minmaxIdx)
    stats = RegionStats(data, minmaxIdx, tranRad);
else
    stats(1,:) = [mean(data) std(data)];
    minmax(length(data)) = 1;
    minmaxIdx = find(minmax);
end
